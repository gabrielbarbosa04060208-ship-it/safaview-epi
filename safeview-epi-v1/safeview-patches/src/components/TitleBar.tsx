// SafeView EPI — TitleBar.tsx
// Idêntica à do SafeView original, apenas título e cores adaptados.
...
export function TitleBar() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      .min-h-screen { min-height: calc(100vh - 28px) !important; }
  ...
  const btnBase: React.CSSProperties = {
    ...
    color: "#cbd5e1", cursor: "pointer", fontSize: 16, transition: "background 0.15s",
  };

  return (
    <div style={{ ...DRAG, position: "fixed", top: 0, left: 0, right: 0, height: 28,
      backgroundColor: "#0a0f1e", borderBottom: "1px solid #1e2a45",
      display: "flex", alignItems: "center", justifyContent: "space-between",
      zIndex: 99999, userSelect: "none" }}>
      <div style={{ ...NODRAG, display: "flex", alignItems: "center", gap: 8, paddingLeft: 12 }}>
        <ShieldIcon />
        <span style={{ color: "#f1f5f9", fontSize: 13, fontWeight: 600, letterSpacing: "0.02em" }}>
          SafeView EPI
        </span>
      </div>
      <div style={{ display: "flex" }}>
        <button style={btnBase} onClick={minimize} title="Minimizar"
          onMouseEnter={e => (e.currentTarget.style.backgroundColor = "#1e2a45")}
          onMouseLeave={e => (e.currentTarget.style.backgroundColor = "transparent")}>
          <svg width="10" height="1" viewBox="0 0 10 1"><rect width="10" height="1" fill="#cbd5e1"/></svg>
        </button>
        <button style={btnBase} onClick={maximize} title={maximized ? "Restaurar" : "Maximizar"}
          ...
          {maximized ? (
            <svg width="11" height="11" viewBox="0 0 11 11">
              <rect x="2" y="0" width="9" height="9" stroke="#cbd5e1" strokeWidth="1.2" fill="none"/>
              <rect x="0" y="2" width="9" height="9" stroke="#cbd5e1" strokeWidth="1.2" fill="#0a0f1e"/>
            </svg>
          ) : (
            <svg width="10" height="10" viewBox="0 0 10 10">
              <rect width="10" height="10" stroke="#cbd5e1" strokeWidth="1.2" fill="none"/>
            </svg>
          )}
        </button>
        <button style={btnBase} onClick={close} title="Fechar"
          ...
          onMouseLeave={e => (e.currentTarget.style.backgroundColor = "transparent")}>
          <svg width="10" height="10" viewBox="0 0 10 10">
            <line x1="0" y1="0" x2="10" y2="10" stroke="#cbd5e1" strokeWidth="1.4"/>
            <line x1="10" y1="0" x2="0"  y2="10" stroke="#cbd5e1" strokeWidth="1.4"/>
          </svg>
        </button>
      </div>
    </div>
